...
export const getCompanyRatingAction =
  (id, page, limit, lang) => async (dispatch) => {
    try {
...
export const createCompanyRatingAction =
  (companyId, star, comment) => async (dispatch) => {
    try {
      dispatch(actions.createCompanyRatingStart());
      const response = await companyApi.postCompanyRating(
        companyId,
        star,
        comment
      );
      if (response.status === 200) {
        ...
        throw new Error("Failed to create company rating");
      }
    } catch (error) {
      ToastAndroid.show(
        "Bạn chưa ứng tuyển tại công ty này",
        ToastAndroid.SHORT
      );
      dispatch(actions.createCompanyRatingFail(error.message));
      throw error;
    }
  };
