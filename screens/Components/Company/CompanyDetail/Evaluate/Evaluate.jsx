...
export default function Evaluate({ company }) {
  ...
  const companyRating = useSelector(
    (state) => state.companyRating.companyRating
  );
  ...
  useEffect(() => {
    if (companyRating && companyRating.data) {
      ...
      if (currentPage === 0) {
        ...
        setListCompanyRating((prev) => [
          ...prev,
          ...companyRating.data.companyRatings,
        ]);
      }
    }
  }, [companyRating]);
    ...
    <View style={styles.container}>
      {total > 0 ? (
        <View>
          ...
          >
            Tổng cộng {total} đánh giá ({averageRated}{" "}
            ...
            renderItem={({ item }) => (
              <View style={styles.item}>
                <View style={styles.left}>
                  <Image
                    source={{
                      uri: item.profileData.avatarPath
                  ...
                  <View style={{ flexDirection: "column", marginLeft: 10 }}>
                    <Text
                      style={{
                        fontWeight: "bold",
                        color:
                          profile.accountId === item.profileData.accountId
                            ? "blue"
                            : "black",
                      }}
                    >
