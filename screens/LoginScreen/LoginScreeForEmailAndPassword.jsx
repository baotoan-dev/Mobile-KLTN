...
export default function LoginScreeForEmailAndPassword() {
  ...
  const [request, response, promptAsync] = Google.useAuthRequest({
    clientId:
      "157728721726-djlk2umnldvfndmaqgms29ob00ecen4a.apps.googleusercontent.com",
    androidClientId:
      "157728721726-41nkgblnad3c3u2eqv5eugbraj79qovd.apps.googleusercontent.com",
  });
    ...
    const handleSignIn = async () => {
      if (response) {
        if (response.type === "success") {
          ...
          if (id_token) {
            try {
              const res = await authCandidate.signInEmail(id_token);
              if (res && res.code === 200) {
                console.log(res);
                SecureStore.setItemAsync("accountId", res.data.accountId);
            ...
            console.warn("No ID token found in response.params");
          }
        } else {
          console.warn("Response type is not success:", response.type);
        }
      } else {
