...
import {
  createCvInformationAction,
  getCvInformationAction,
} from "../../../../redux/store/CvInFormation/cvInformationSlice";
...
export default function PersonalInformation(prop) {
  ...
  useEffect(() => {
    if (cvInformation && cvInformation.data) {
      ...
      setAvatarPath(
        cvInformation?.data?.avatar ? cvInformation?.data?.avatar : null
      );
    }
  }, []);
  ...
  const handleSavePersonalInformation = () => {
    const formData = createCvInformation(
      name,
      email,
      phone,
      address,
        ...
        >
          <Image
            source={
              cvInformation &&
              cvInformation?.data &&
              cvInformation?.data?.avatar
                ? { uri: cvInformation?.data?.avatar }
                : listImage && listImage.uri
                ? { uri: listImage.uri }
