...
export default function Skill(prop) {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [cvIndexParent, dispatch, cvExtraInformation]);
  ...
  const handleDeleteExtraInformation = async (id) => {
    ...
    const newListExtraInformation = listSkill?.moreCvExtraInformations?.filter(
      (item) => +item.id !== +id
    );
    ...
    const updatedListOtherInformation = [
      ...listOtherInformation,
      newCreateCvExtraInformation,
    ];

    if (updatedListOtherInformation.length > 0) {
      setLoading(true);
            ...
            <View style={styles.item} key={index}>
              ...
              >
                ...
                <View style={{ marginLeft: 10 }}>
                  ...
                  <Text
                    numberOfLines={1}
                    style={{ fontSize: 12, marginTop: 2, color: "gray" }}
                  >
